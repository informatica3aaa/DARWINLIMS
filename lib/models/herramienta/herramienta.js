  ...
  // ``
    ...
    static async getCondiciones(data){
      return await new sql.ConnectionPool(sqlConfig).connect().then(pool => {
        ...
        ,[paragraph_4]
        ,[active]
        ,UPPER([title]) AS title from [general_conditions] where active=@active and id <>  1 ORDER BY ID ASC
        `)
      }).then(result => {
        let rows = result.recordset;
        sql.close();
    ...
    static async getBalanzas(offset, limit ){
      return await new sql.ConnectionPool(sqlConfig).connect().then(pool => {
        return pool.request()
    ...
    static async balEditar(data){
          return await new sql.ConnectionPool(sqlConfig).connect().then(pool => {
        return pool.request()
        .input('id', sql.Int, data.id)
        .input('name', sql.VarChar, data.name)
        .input('bitrate', sql.Int, data.bitrate)
        .input('parity', sql.VarChar, data.parity)
        .input('stopbit', sql.Int, data.stopbit)
        .input('databit', sql,Int, data.databit)
        .input('portname', sql.VarChar, data.portname)
        .input('calibracion', sql.DateTime, data.calibracion)
        .input('patron_a', sql.Decimal, data.patron_a)
        .input('patron_a_min', sql.Decimal, data.patron_a_min)
        .input('patron_a_max', sql.Decimal, data.patron_a_max)
        .input('patron_b', sql.Decimal, data.patron_b)
        .input('patron_b_min', sql.Decimal, data.patron_b_min)
        .input('patron_b_max', sql.Decimal, data.patron_b_max)
        .input('patron_c', sql.Decimal, data.patron_c)
        .input('patron_c_min', sql.Decimal, data.patron_c_min)
        .input('patron_c_max', sql.Decimal, data.patron_c_max)
        .query(`
        UPDATE [scales]
        SET [name] = @name
            ,[bitrate] = @bitrate
            ...
            ,[patron_a] = @patron_a
            ,[patron_a_min] = @patron_a_min
            ,[patron_a_max] = @patron_a_max
            ,[patron_b] = @patron_b
            ,[patron_b_min] =@patron_b_min
            ,[patron_b_max] = @patron_b_max
            ,[patron_c] = @patron_c
            ,[patron_c_min] = @patron_c_min
            ,[patron_c_max] = @patron_c_max OUTPUT Inserted.*
        WHERE id = @id `)
      }).then(result => {
        let rows = result.recordset;
        sql.close();
    ...
    static async balCalibracion(id, offset, limit){
        ...
        .input('offset', sql.Int, Number(offset))
        .input('limit', sql.Int, Number(limit))
        .query(`SELECT CONCAT(us.[name] , ' ' , us.[lastname_f] , ' ', us.[lastname_m]) as usuario
            ,sca.[name] as scale_name
            ,cal.[id]
            ,cal.[scale_id]
            ,cal.[user_id]
            ,cal.[weight1]
            ...
            ,cal.[created]
            ,cal.[modified]
            FROM [calibrations] cal INNER JOIN
            users us ON cal.user_id = us.id  INNER JOIN 
            scales sca ON cal.scale_id = sca.id
            WHERE cal.[scale_id] =  @id
            order by cal.[modified] desc
    ...
    static async balCalibracionCont(id, offset, limit){
      return await new sql.ConnectionPool(sqlConfig).connect().then(pool => {
    return pool.request()
    .input('id', sql.Int, id)
    .query(`SELECT 
...
static async editCurrencies(id, valor){
  console.log("id", id);
  return await new sql.ConnectionPool(sqlConfig).connect().then(pool => {
    return pool.request()
    .input('id', sql.Int, id)
    ...
    [user_modifier_id]=542 OUTPUT Inserted.*
    WHERE id = @id  `)
  }).then(result => {
    let rows = result.recordset;
    sql.close();
