...
class Cotizaciones {
    ...
    static async getDetallesCotizacion(id){
        ...
        LEFT JOIN methods me ON ass.method_id = me.id where quotation_id =@id order by tipo, assay_name asc`)
          }).then(result => {
            let rows = result.recordset;
            sql.close();
            return rows;
...
break;  
    ...
    static async consultaEstadoCotizacion(data){
      return await new sql.ConnectionPool(sqlConfig).connect().then(pool => {
        return pool.request()
        .input('id', sql.Int, Number(data.id))
        .query(`SELECT *  FROM [quotations] where ap_ventas = 1 and ap_prod = 1 and id=@id`)
      }).then(result => {
        let rows = result.recordset;
        sql.close();
        return rows;
    ...
    static async getCotizacionXuser(data, user){
        ...
        return await new sql.ConnectionPool(sqlConfig).connect().then(pool => {
          ...
          .query(`SELECT [id] FROM [quotations] where active=@active and user_creator_id=@user_id`)
        }).then(result => {
          let rows = result.recordset;
          sql.close();
          return rows;
...
static async getDetallesAdmin(){
  return await new sql.ConnectionPool(sqlConfig).connect().then(pool => {
    return pool.request()
    .query(`SELECT * FROM [dbo].[assays]  where active = 1 and [is_admin_code] = 1`)
    }).then(result => {
    let rows = result.recordset;
    sql.close();
    return rows;
...
static async getDestinatario(company_id, modulo){
    ...
    .query(`SELECT * FROM [dbo].[destinatarios] where company_id = @company_id and active = 1 and modulo=@modulo`)
    }).then(result => {
    let rows = result.recordset;
    sql.close();
    return rows;
...
static async getStatusEstados(){
  return await new sql.ConnectionPool(sqlConfig).connect().then(pool => {
  return pool.request()
  .query(`SELECT COUNT(quo.id) as cant, year(quo.created) as anio , st.name as estado ,quo.state_id,
          datepart(quarter, quo.created) as trimestre 
          FROM [dbo].[quotations] quo  INNER JOIN states st ON quo.state_id = st.id
          where 
          YEAR(quo.created) = YEAR(CURRENT_TIMESTAMP) 
