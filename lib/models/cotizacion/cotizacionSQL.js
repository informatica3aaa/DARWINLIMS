...
static async getDestinatario(company_id){
    ...
    return pool.request()
    .input('company_id', sql.Int, Number(company_id))
    .query(`SELECT * FROM [dbo].[destinatarios] where company_id = @company_id and active = 1`)
    }).then(result => {
    let rows = result.recordset;
    sql.close();
    return rows;
...
static async addDestinatario(company_id, mail, name, user_creator_id, telefono, modulo, cargo){
  return await new sql.ConnectionPool(sqlConfig).connect().then(pool => {
  return pool.request()
  .input('company_id', sql.Int, Number(company_id))
