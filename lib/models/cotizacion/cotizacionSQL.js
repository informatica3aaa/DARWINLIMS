...
static async ListarQuoByCompany(id){
  return await new sql.ConnectionPool(sqlConfig).connect().then(pool => {
    ...
    .input('id', sql.Int, Number(id))
    .query(`
    select quo.quotation_number, quo.id, com.rut, quo.project_id, pr.name from quotations quo inner join companies com on com.id= quo.company_id  left join projects pr on quo.project_id = pr.id  
