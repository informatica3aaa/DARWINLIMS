...
break;  
    ...
    static async updateAccion(data, usuario){
      switch(data.accion){
          case 'aprobar_venta':
              ...
              .input('user_id', sql.Int, Number(usuario.user_id))
    ...
    static async cambiaEstadoCotizacion(data){
      return await new sql.ConnectionPool(sqlConfig).connect().then(pool => {
        return pool.request()
        .input('id', sql.Int, Number(data.id))
        .input('active', sql.Bit, Number(data.active))
    ...
    static async getServiciosAnaliticos(data){
      return await new sql.ConnectionPool(sqlConfig).connect().then(pool => {
        return pool.request()
        .input('active', sql.Bit, Number(data.active))
        .input('assay_type_id', sql.Int, Number(data.assay_type_id))
        .query(`SELECT 
                      ass.[id]
                      ,ass.[assay_type_id]
              ...
              WHERE ass.active=@active and ass.assay_type_id= @assay_type_id`)
        }).then(result => {
        let rows = result.recordset;
        sql.close();
        return rows;
    ...
    static async getFasesServiciosAnaliticos(data){
      return await new sql.ConnectionPool(sqlConfig).connect().then(pool => {
        ...
        .query(`SELECT fa.[id]
                ...
                ,fa.[assay_id]
                ,fa.[order]
                FROM [lims01].[dbo].[phase_assays] fa LEFT JOIN 
                phases fh On fa.phase_id = fh.id
                where fa.assay_id = id order by fa.[order] asc`)
        }).then(result => {
        let rows = result.recordset;
        sql.close();
        return rows;
