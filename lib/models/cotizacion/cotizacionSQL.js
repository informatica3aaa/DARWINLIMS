...
static async getDestinatario(company_id){
    return await new sql.ConnectionPool(sqlConfig).connect().then(pool => {
    return pool.request()
    .input('company_id', sql.Int, Number(company_id))
    .query(`SELECT * FROM [lims01].[dbo].[company_mails] where company_id = @company_id and active = 1 and [name] is not null`)
    }).then(result => {
    let rows = result.recordset;
    sql.close();
    return rows;
...
static async addDestinatario(company_id, mail, name, user_creator_id, telefono){
  return await new sql.ConnectionPool(sqlConfig).connect().then(pool => {
  return pool.request()
  .input('company_id', sql.Int, Number(company_id))
  ...
  .query(`INSERT INTO [dbo].[company_mails]
          ([company_id]
          ,[mail]
          ,[name]
          ,[user_creator_id]
