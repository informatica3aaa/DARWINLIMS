...
static async ListarCompanys(){
  return await new sql.ConnectionPool(sqlConfig).connect().then(pool => {
    return pool.request()
    .query(`
    select DISTINCT(UPPER(com.fantasy_name)), com.rut, com.id from quotations quo inner join companies com on com.id= quo.company_id where quo.active =1 and quo.state_id = 2
